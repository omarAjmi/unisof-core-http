...
public class ExponentialBackoff implements RetryStrategy {
    ...
    public ExponentialBackoff(int maxRetries, Duration baseDelay, Duration maxDelay) {
        ...
        if (baseDelay.isZero()) {
            throw LogUtil.logExceptionAsError(log, new IllegalArgumentException("'baseDelay' cannot be 0."));
        }

        if (baseDelay.compareTo(maxDelay) > 0) {
            throw LogUtil
                    .logExceptionAsError(log, new IllegalArgumentException("'baseDelay' cannot be greater than 'maxDelay'."));
        }

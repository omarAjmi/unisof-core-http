...
public class PercentEscaper {
    ...
    private static final Logger log = LoggerFactory.getLogger(PercentEscaper.class);
    ...
    PercentEscaper(String safeCharacters, boolean usePlusForSpace) {
        ...
        if (usePlusForSpace && safeCharacters != null && safeCharacters.contains(" ")) {
            throw LogUtil.logExceptionAsError(log, new IllegalArgumentException(
        ...
        if (!CoreUtil.isNullOrEmpty(safeCharacters)) {
     ...
     */
    private static int getCodePoint(String original, int index, int end, Logger log) {
        ...
        if (!Character.isSurrogate(char1)) {
            ...
            if (index == end) {
                throw LogUtil.logExceptionAsError(log, new IllegalStateException(
    ...
    public String escape(String original) {
        ...
        if (CoreUtil.isNullOrEmpty(original)) {
            return original;
        }
         ...
         */
        while (index < end) {
            int codePoint = getCodePoint(original, index, end, log);
